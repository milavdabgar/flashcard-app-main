...
def dashboard():
    ...
    if g.user:
        try:
            ...
            images = []
            ratings = []
            decks_studied, averages, points, times = get_performance(
                get_user_id(session["user"])
            )
            for deck in decks_studied:
                scores.append(
                    get_score_breakdown(get_user_id(session["user"]), deck[0])
                )
            for deck in decks_studied:
...
def delete_card1(deck_id):
    if g.user:
        try:
            return render_template(
                "delete_card.html", cards=get_cards(deck_id), deck=get_deck(deck_id)
            )
        except:
            return render_template("error.html")
    return redirect(url_for("signin"))
